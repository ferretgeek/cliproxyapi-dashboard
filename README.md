...
- **看状态** — 服务是否在跑、CPU / 内存 / 磁盘、上游构建版本与最新 Release、连接诊断。
- **看用量和花费** — 实时请求量来自日志；已有的 Token 消耗与成本历史从本地兼容快照读取，当前界面展示历史合计。估算单价可从 OpenRouter 自动同步，也可以关闭同步后手动填写。
- **看日志** — 增量解析、错误高亮与关键字筛选，不用 SSH 进去 `tail`。
...
- **换主题** — 天空、薄荷、玫瑰、沙色四套浅色配色 + 深蓝灰暗色，右上角切换并记住选择；手机上是完整可用而不只是能打开。
...
**日志是增量解析的。** 避免每次刷新重读整个文件，每轮扫描设置字节和时间预算，尾部读取也有大小上限；状态持久化走原子写入，上游不可用时退避重试，备份按数量、时长和体积三重上限自动淘汰。
