...
- 状态、资源、日志和健康检查由后台快照采集，HTTP 请求不再直接执行慢采集；状态 / 资源 / 健康检查标注采集时间及过期状态。
...
- 本机没有 Docker CLI；GitHub Actions 已实际执行 Compose 配置检查、镜像构建、容器网络 / 认证 / 数据卷重建 / 上游中断恢复测试，全部通过。
- CI：[`34756260081`](https://github.com/ferretgeek/cliproxyapi-dashboard/actions/runs/34756260081)，候选提交 `47bdd16532daf3ca7014881fed117446bbd559ad`。Ubuntu / Windows × Python 3.11 / 3.13 四组测试全部通过。
- 容器测试日志：`requests_during_outage=200`、`max_status_seconds=0.008`、`volume_recreation=pass`。这是 CI 单次模拟场景结果，不是生产性能承诺。
- 手动浏览器检查配置默认只读、保存按钮禁用、YAML 校验有效；深浅色与手机截图使用模拟上游数据。
- `tests/preview_server.py` 为本地 UI 测试；`tests/fake_upstream.py` 为 Docker 联调服务器，两者均不代表真实 CLIProxyAPI 生产环境。
